...
export default function ContainerPosts() {
    ...
    return (
        <FlatList
            ...
            data={newData}
            renderItem={({ item }) => {
                return <Post postId={item.id} ownUserId={item.userId} imageSrc={{ uri: item.image }} content={item.content} personSrc={{ uri: item.userImage }} namePerson={`${item.givenName} ${item.givenSurname ? item.givenSurname : ""}`} locationPerson={item.country} timePost={item.timeAgo} likeCount={item.likeCount} />;
            }}
