...
export default function ContainerPosts() {
    ...
    return <FlatList style={styles.container} data={newData} renderItem={({item}) => {
        return <Post imageSrc={{uri: item.image}} content={item.content} personSrc={{uri: item.userImage}}
                     namePerson={`${item.givenName} ${item.givenSurname}`} locationPerson={item.country}
                     timePost={data[0].timePost}/>
